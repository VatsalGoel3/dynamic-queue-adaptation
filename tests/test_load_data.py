...
def test_preprocess_catalog_normalizes_numeric_features_and_round_trips(
    tmp_path: Path,
) -> None:
    raw_catalog = generate_synthetic_catalog(num_tracks=12, seed=19)
    ...
    for column in normalized_columns:
        assert pd.api.types.is_float_dtype(processed[column])
        assert processed[column].between(0.0, 1.0).all()
